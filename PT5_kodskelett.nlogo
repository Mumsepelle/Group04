...
to-report create-belief [b-type content]
  if show-intentions [type b-type show content]
...
to-report exist-beliefs-of-type [b-type]
  let blfs filter [i -> first i = b-type ] beliefs
  ifelse empty? blfs [report false] [report true]
end

;;; Returns all beliefs of b-type in a list
to-report beliefs-of-type [b-type]
  report filter [ i -> first i = b-type ] beliefs
end

;;; Returns the agent's belief of a certain type and removes it
to-report get-belief [b-type agent]
  ifelse exist-beliefs-of-type b-type
  [let bel first filter [first beliefs = b-type and item 1 beliefs = agent] beliefs
   remove-belief bel
   report bel
  ]
  [report false]
...
;;;locals [myInt]  ;; first intentions
  ...
  if show-intentions [set label myInt] ;; Just for debugging.
  if show-intentions [show myInt] ;; Just for debugging.
